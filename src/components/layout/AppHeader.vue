<template>
  <header class="app-header">
    ...
    <div class="header-right">
      ...
      >
        <svg v-if="isDarkModeActive" viewBox="0 0 24 24" width="16" height="16">
...
const darkModeTooltip = computed(() => {
  const mode = themeManager.isDarkMode.value
  if (mode === 'auto') return '主题：自动（跟随系统）'
  if (mode === 'dark') return '主题：黑暗模式'
  return '主题：明亮模式'
})
